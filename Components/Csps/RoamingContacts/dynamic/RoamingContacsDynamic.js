...
export default function ProviderContactDynamic ({params}) {
    ...
    useEffect(()=>{
        if (/^[0-9]*$/.test(params.id) ) {
            const url = `http://172.19.128.128:3060/api/roamingoperator/${params.id}`
    ...
    const onCloseEdit = () => {
        if (areThereChanges) {
            ...
            if (areYouSure) {
                if (params.id === "new") {
                    router.push("/csps/roamingcontact")
                } else {
                    setCanEdit(false)
    ...
    const onSave = async () => {
        ...
        try {
            ...
            const api = "http://172.19.128.128:3060/api/roamingoperator"
                ...
                <div>
                    ...
                    <div className="bg-red-500 text-white">
                        ...
                        <div>
                            ...
                            <input  className={`p-2 w-full text-gray-800 ${canEdit?"bg-white":"bg-gray-300"}`} type="text" name={"country"} value={dataProvider.country} onChange={(e)=>canEdit&&onChangeForm(e)} />
                        </div>
                        <div>
                            <div className="p-2">
                                Operador:
                            </div>
                            <input  className={`p-2 w-full text-gray-800 ${canEdit?"bg-white":"bg-gray-300"}`} type="text" name={"operator"} value={dataProvider.operator} onChange={(e)=>canEdit&&onChangeForm(e)} />
                        </div>
                        <div>
                            <div className="p-2">
                                Telefono:
